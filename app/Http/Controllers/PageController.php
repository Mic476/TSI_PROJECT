...
class PageController extends Controller
{
    ...
    public function dashboardHrd()
    {
        ...
        if (!$data['user_login'] && auth()->check()) {
            $data['user_login'] = auth()->user();
        }

        $data['users_rules'] = $data['user_login']
        ...
        $data['gmenu'] = DB::table('sys_gmenu')
            ->join('sys_auth', 'sys_gmenu.gmenu', '=', 'sys_auth.gmenu')
            ->whereIn('sys_auth.idroles', $data['users_rules'])
            ->where('sys_gmenu.isactive', '1')
            ->select('sys_gmenu.*')
            ->distinct()
            ->orderBy('urut')
        ...
        $data['url_menu'] = 'dashboard-hrd';
        $data['title_group'] = 'HRD';
        $data['title_menu'] = 'Dashboard HRD';
        $data['dmenu'] = 'dashhd';

        return view('pages.dashboard.hrd', $data);
    }
    ...
    public function dashboardPetugas()
    {
        ...
        $normalizedRoles = array_map('strtolower', array_map('trim', $data['users_rules']));
        $isOffice = in_array('ptgoff', $normalizedRoles, true);
        $isPU = in_array('ptgspu', $normalizedRoles, true) || in_array('ptgpu', $normalizedRoles, true);
        $activeRole = null;
        ...
        $roleGroupUserIds = [];
        if ($activeRole) {
            $roleGroupUserIds = DB::table('users')
                ->whereRaw("FIND_IN_SET(?, REPLACE(idroles, ' ', ''))", [$activeRole])
                ->where('isactive', '1')
                ->pluck('id')
                ->toArray();
        }

        if (empty($roleGroupUserIds) && !empty($data['user_login']->id)) {
            $roleGroupUserIds = [(int) $data['user_login']->id];
        }

        $nonPeriodicBaseQuery = DB::table('pl_non_periodic')
            ...
            ->select(
                ...
                'pl_non_periodic.created_at',
                'pl_non_periodic.hrd_note',
                'pl_non_periodic.worker_id',
                'ms_area.nama_area as area_name'
            )
            ->whereIn('pl_non_periodic.request_status', ['pengerjaan', 'revisi'])
            ->where('pl_non_periodic.is_active', '1');

        if (!empty($roleGroupUserIds)) {
            $nonPeriodicBaseQuery->whereIn('pl_non_periodic.worker_id', $roleGroupUserIds);
        } else {
            $nonPeriodicBaseQuery->whereRaw('1 = 0');
        }

        $periodicBaseQuery = DB::table('pl_periodic_items')
            ...
            ->whereNull('pl_periodic_items.realization_date')
            ->whereNotNull('pl_periodic_items.planned_date');

        if (!empty($roleGroupUserIds)) {
            $periodicBaseQuery->whereIn('pl_periodic_detail.worker_id', $roleGroupUserIds);
        } else {
            $periodicBaseQuery->whereRaw('1 = 0');
        }

        $nonPeriodicTasks = (clone $nonPeriodicBaseQuery)
            ->orderBy('pl_non_periodic.created_at', 'desc')
            ->limit(20)
            ->get();

        $periodicItems = (clone $periodicBaseQuery)
        ...
        $pendingVerificationQuery = DB::table('pl_non_periodic')
            ->where('request_status', 'verifikasi')
            ->where('is_active', '1');

        if (!empty($roleGroupUserIds)) {
            $pendingVerificationQuery->whereIn('worker_id', $roleGroupUserIds);
        } else {
            $pendingVerificationQuery->whereRaw('1 = 0');
        }

        $data['pending_verification'] = $pendingVerificationQuery->count();

        $data['non_periodic_tasks'] = $nonPeriodicTasks;
        $data['periodic_items'] = $periodicItems;

        $data['url_menu'] = 'dashboard-petugas';
        $data['title_group'] = 'Petugas';
        $data['title_menu'] = 'Dashboard Petugas';
        $data['dmenu'] = 'dashptg';
        $data['user_role'] = $isOffice ? 'Office' : ($isPU ? 'PU' : 'Unknown');

        return view('pages.dashboard.petugas', $data);
    }
}
