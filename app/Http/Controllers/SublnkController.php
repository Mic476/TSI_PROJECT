    ...
    public function update($data)
    {
        ...
        foreach (
            ...
            if ($item['field'] == 'email') {
                ...
                // ambil rules aslinya
                $rules = is_array($item['validate']) ? $item['validate'] : explode('|', $item['validate']);
                // hapus rule 'required' dari array
                $rules = array_filter($rules, function ($r) {
                    return trim($r) !== 'required';
                });
                // simpan kembali ke validasi
                $validate[$item['field']] = $rules;
            } else {
                $validate[$item['field']] = $item['validate'];
            }
            // check data type multiple
