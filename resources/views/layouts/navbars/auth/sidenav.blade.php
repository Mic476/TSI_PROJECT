<aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 "
    ...
    <div class="collapse navbar-collapse w-auto " id="sidenav-collapse-main" style="height: 100%;">
        <ul class="navbar-nav">
            {{-- Dashboard Petugas Link --}}
            @if (in_array('ptgoff', $users_rules, true) || in_array('ptgspu', $users_rules, true))
                <li class="nav-item">
                    <a class="nav-link blank {{ request()->is('dashboard-petugas') ? 'active' : '' }}"
                        href="{{ route('dashboard.petugas') }}">
                        <div class="icon icon-shape icon-sm text-center d-flex align-items-center justify-content-center">
                            <i class="ni ni-chart-pie-35 text-primary text-sm opacity-10"></i>
                        </div>
                        <span class="nav-link-text ms-1 py-1">Dashboard Petugas</span>
                    </a>
                </li>
            @endif
            {{-- LOOPING GMENU --}}
