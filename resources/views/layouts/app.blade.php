...
<body class="{{ $class ?? '' }}">
    ...
    <script>
        (() => {
            const KEY_FORCE = 'msj_force_sidenav'; // 1 = paksa mode toggleable di semua layar
            const KEY_HIDE = 'msj_hide_sidenav'; // 1 = sidenav disembunyikan
            const body = document.body;
            const btn = document.getElementById('toggleSidenavFromProfile');
            const text = document.getElementById('toggleSidenavText');

            function apply() {
                const force = localStorage.getItem(KEY_FORCE) === '1';
                const hide = localStorage.getItem(KEY_HIDE) === '1';

                body.classList.toggle('msj-force-sidenav', force);
                body.classList.toggle('g-sidenav-hidden', hide);

                if (text) text.textContent = hide ? 'Show Menu' : 'Hide Menu';
            }
            // default: force mode ON (kalau kamu ingin default seperti 5199)
            if (localStorage.getItem(KEY_FORCE) === null) localStorage.setItem(KEY_FORCE, '1');
            if (localStorage.getItem(KEY_HIDE) === null) localStorage.setItem(KEY_HIDE, '0');
            apply();
            if (btn) {
                btn.addEventListener('click', (e) => {
                    e.preventDefault();

                    const isHidden = body.classList.contains('g-sidenav-hidden');
                    localStorage.setItem(KEY_HIDE, isHidden ? '0' : '1');
                    apply();
                });
            }
        })();
